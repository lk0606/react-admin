            ...
            tableData: [
                {
                    ...
                    tableName:
                        '【202004271245】该分钟应用访问异常的数据库拓扑信息',
                },
            ],
        },
    },
        ...
        columnName: [
            {
                ...
                key: '应用名',
                render: (text) => (
                    <a href="http://antmonitor.alipay.com/ob/tenant?clusterGroup=obantloan&cluster=obantloan00&tenant=zarcore000_2132">
                        {text}
                    </a>
                ),
            },
    ...
    if (item.table) {
        ...
        if (item.tableData[0].data.length > 0) {
            ...
            item.tableData[0].data.forEach((subItem, subIndex) => {
                const [et2, url] = subItem['tenant_topology服务器'].split(
                    '#url#'
                )
                let columnNameItem = {}
                let dataSourceItem = {
